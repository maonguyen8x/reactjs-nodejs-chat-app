...
const UserListItem = (props) => {
  ...
  const getAvatarColor = () => {
    ...
    if (props.userId !== undefined || props.id !== undefined) {
      axios
        .get("/api/users/" + avatarId)
        .then((newContact) => {
          if (
            newContact.data.avatarColor !== null &&
            newContact.data.avatarColor !== undefined
          ) {
  ...
  const editContact = (event) => {
    ...
    axios
      .post("/api/users/update-contact/" + props.name, data, config)
      .then((response) => {
        history.push("/all");
      })
      .catch((err) => console.log(err));
  ...
  let deletePopup =
    deleteUserPopup === true ? (
      <Popup clearPopupState={() => clearPopuState()}>
        ...
        <button
          className="contact-button-delete margin-xs"
  ...
  if (props.selectContact !== undefined) {
    return (
      <div className="user-list-item padding-20 row">
        ...
        <div
          className="user-list-img-col clickable"
          onClick={() => props.selectContact(props.id)}
        >
          <svg
            aria-hidden="true"
            focusable="false"
            data-prefix="fas"
            data-icon="user"
            className="svg-inline--fa fa-user fa-w-14 svg-avatar-nav"
            role="img"
            viewBox="0 0 448 512"
          >
            <path
        ...
        <div className="col">
          ...
          <div
            className="row height-50 space-between align-center clickable"
            onClick={() => props.selectContact(props.id)}
          >
            <h3>{props.name}</h3>
          </div>
        ...
        </div>
        {
          ...
          props.type === "USER_LIST_GROUP" ? (
            ...
            <div className="user-list-button-col">
              {props.alreadyAdded === "Already a friend" ? (
                ""
              ) : (
                <button
                  onClick={apiCall}
