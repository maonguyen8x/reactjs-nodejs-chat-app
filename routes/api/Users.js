...
router.get("/:id", isLoggedIn, (req, res) => {
  // const foundUser = User.find((us) => us.id === Number(req.params.id));
  // if(!foundUser) {
        ...
        .then((contact) => {
          ...
          if (
            user.contacts.filter(
              (contacts) => contacts.user.toString() === contact.id
            ).length > 0
          ) {
            return;
          }
...
router.post("/update-contact/:username", isLoggedIn, (req, res) => {
  ...
  User.updateOne(
    {
      _id: req.user._id,
      contacts: { $elemMatch: { username: req.params.username } },
    },
    {
      $set: {
...
router.get("/:id/delete", isLoggedIn, (req, res) => {
  if (
    req.user.contacts !== null &&
    req.user.contacts !== undefined &&
    req.user.contacts.length !== 0
  ) {
